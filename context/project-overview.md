...
JobPilot is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile using Deepseek v4 pro. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
- Two options on upload:
  - "Extract from Resume" → Deepseek v4 pro parses resume and auto-fills profile form fields
  - "Skip" → resume stored as-is, profile unchanged
- User can manually edit any profile field at any time
- User can generate a clean professional PDF resume from their current profile data using Deepseek v4 pro
...
- Deepseek v4 pro scores each job 0-100 against user profile
...
- Match score section:
  ...
  - Match reason paragraph from Deepseek v4 pro
...
- Deepseek v4 pro synthesizes all extracted content into structured dossier
- Dossier displayed on job details page
- If company site cannot be found — Deepseek v4 pro generates best dossier from company name and job description alone
...
- Resume PDF upload with optional profile auto-fill via Deepseek v4 pro
- Resume PDF generation from profile data using Deepseek v4 pro
- Adzuna API job discovery — searches by title and location, category filtered to IT jobs
- Deepseek v4 pro job matching with score, reason, matched skills, missing skills
- Job details page with full structured description
- Company Research Agent — single Browserbase session browses company public pages, Deepseek v4 pro builds dossier
...
- Deepseek v4 pro match scores feel accurate and the reasoning makes sense
