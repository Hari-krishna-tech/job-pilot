...
| AI model                       | Deepseek v4 pro            | Matching, research synthesis, extraction         |
...
├── agent/
│   ├── adzuna.ts                          → Adzuna API job discovery + Deepseek v4 pro scoring
│   ├── research.ts                        → Company research — Browserbase + Stagehand + Deepseek v4 pro
│   ├── matcher.ts                         → Deepseek v4 pro job matching logic
│   ├── extractor.ts                       → Deepseek v4 pro job description extraction + structuring
...
Adzuna API returns job listings
        ↓
Deepseek v4 pro scores each job against user profile
        ↓
...
Navigates to company homepage + sub pages
        ↓
Deepseek v4 pro synthesizes dossier from extracted content
        ↓
...
API route in app/api/resume/
        ↓
Deepseek v4 pro processes content
        ↓
...
| match_reason       | text        | Deepseek v4 pro explanation                             |
...
const stagehand = new Stagehand({
  ...
  modelClientOptions: { apiKey: process.env.DEEPSEEK_API_KEY! },
});
...
try {
  ...
  // Log and continue — Deepseek v4 pro will synthesize from what was found
...
- Company research always returns a dossier — even if browser research fails, Deepseek v4 pro synthesizes from company name and job description alone. Never return empty.
