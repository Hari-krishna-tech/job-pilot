...
- Deepseek reads extracted text and returns structured JSON matching all profile field names
...
- Deepseek generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders Deepseek output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - Deepseek scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from Deepseek
...
- Derive company homepage URL by following the Adzuna redirect with server-side fetch() — no browser needed for this step:
  ...
  - If Stagehand gets no meaningful content (oneLiner and productSummary empty) — skip browser research entirely, proceed to Deepseek synthesis with job description and profile only
...
- Close Browserbase session after homepage + max 3 sub-pages
  **Deepseek synthesis (runs after browser closes):**
...
- Always return a dossier — never fail silently. If browser research failed, Deepseek synthesizes from job description and profile alone.
