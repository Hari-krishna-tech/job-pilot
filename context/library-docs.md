...
- Adzuna description is a snippet — Deepseek scores from it, not a full description
...
const stagehand = new Stagehand({
  ...
  model: { modelName: "deepseek/deepseek-chat", apiKey: process.env.DEEPSEEK_API_KEY! },
  disablePino: true,
});
...
Three-step process: homepage extraction → sub-page extraction → Deepseek synthesis.
...
// Step 3 — Deepseek synthesis (after browser closes)
...
- Model is always `deepseek-chat` — never use other models
...
**Check first:** Check AGENTS.md for an installed Deepseek skill. The skill will have the latest API patterns and model capabilities.
...
- Model string is always `'deepseek-chat'` — never use other model names
...
- Server-side only — never import in client components
- `pdfData.text` is raw unformatted text — Deepseek handles the structure extraction
